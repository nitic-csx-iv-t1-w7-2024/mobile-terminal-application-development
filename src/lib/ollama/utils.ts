 ...
 */
const fetchWithHeaders = async (fetch: Fetch, url: string, options: RequestInit = {}): Promise<Response> => {
 ...
 * @returns {Promise<Response>} - The fetch response
 */
export const del = async (fetch: Fetch, host: string, data?: Record<string, unknown>): Promise<Response> => {
  const response = await fetchWithHeaders(fetch, host, {
    method: 'DELETE',
