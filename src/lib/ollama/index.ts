...
import { AbortableAsyncIterator, parseJSON, post } from './utils';
...
export class Ollama {
   ...
   */
  async generate(request: GenerateRequest): Promise<GenerateResponse | AbortableAsyncIterator<GenerateResponse>> {
    if (request.images) {
   ...
   * an AbortableAsyncIterator that yields response messages.
   */
  async pull(request: PullRequest): Promise<ProgressResponse | AbortableAsyncIterator<ProgressResponse>> {
   ...
   * an AbortableAsyncIterator that yields response messages.
   */
  async push(request: PushRequest): Promise<ProgressResponse | AbortableAsyncIterator<ProgressResponse>> {
...
export const ollama = new Ollama({
  host: process.env['EXPO_PUBLIC_OLLAMA_HOST'] ?? 'http://ollama:11434',
});
